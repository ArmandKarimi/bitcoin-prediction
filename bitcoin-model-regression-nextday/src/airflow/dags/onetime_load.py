...
def fetch_bitcoin_data():
    try:
        ...
        df = btc.history(start="2014-01-01", end=datetime.now().strftime("%Y-%m-%d"))
        df = df[['Open', 'High', 'Low', 'Close', 'Volume']]
        df.reset_index(inplace = True)
        # ✅ Fix: Convert 'Date' to string format `YYYY-MM-DD`
        df['Date'] = df['Date'].dt.strftime('%Y-%m-%d')  # Removes time & timezone
        # ✅ Fix: Round numeric values to 8 decimal places
...
with DAG(
    ...
    fetch_data >> upload_data >> load_to_redshift
