...
from config import NAME, START, END

def load_data(name = NAME, start = START, end = END):
    ...
    df['Label'] = (df['Change'] > 0).astype(int)  # Faster than apply()
...
if __name__ == "__main__":
    df = load_data(name = NAME, start = START, end = END).head()
    print(df.head())
    print(df.columns)
