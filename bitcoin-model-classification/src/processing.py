...
def moving_avg_normalization(data, window_size=24):
    ...
    labels = data['Label']
    normalized_data = normalized_data.drop(['Label'], axis = 1)
    #normalized_data = pd.concat([normalized_data, data['Label']], axis = 1)
    return normalized_data, labels


#------- create sequences -------
def create_sequences(X, y, seq_length=SEQ_LENGTH, pred_length=PRED_LENGTH):
    """
    ...
    for i in range(len(X) - seq_length - pred_length + 1):
        ...
        targets.append(y.iloc[i+seq_length:i+seq_length+pred_length].values)
